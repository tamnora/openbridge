...
- CLI: `init`, `server`/`start` (`--detach`), `stop`, `status`, `logs` (`--follow`),
  `bridge` (`--api --token --id --name`), `import`, `reset`, `autostart install|remove`,
  `doctor`, `version`, `help`.
- Casa portable (`paths.js`): `config.json`, `app.json`, `folders.json`, `data/`, `logs/`.
...
- Tests (`node --test`): smoke, auth, API y `safeJoinWorkspace` (11/11). CI Windows/Linux/macOS.
- `docs/ARCHITECTURE.md`, `CHANGELOG.md`, `AGENTS.md`, README, LICENSE, `.gitignore`.
- Verificado: `init`+`doctor`, `server`, login, `chat.php`, `bootstrap`, SSE por túnel real y
  `ping` público. Nombre npm `openbridge` **libre**.
...
- [x] `init`: si el workspace es la propia casa, **excluir** `data/`, `logs/`.
...
- [ ] Publicar en **npm público** como `openbridge` (nombre libre) o `@tamnora/openbridge` privado.
...
2. Copiar la **URL pública** que imprime y abrirla en el celular.
